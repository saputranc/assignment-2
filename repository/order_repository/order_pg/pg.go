...
func (orderPG *orderPG) DeleteOrder(orderPayload entity.Order, itemPayload []entity.Item) errs.Error {
	tx, err := orderPG.db.Begin()
	...
	_, err = tx.Exec(deleteOrderByIdQuery, orderPayload.OrderId, orderPayload.OrderedAt, orderPayload.CustomerName)
	...
	for _, eachItem := range itemPayload {
		_, err = tx.Exec(deleteItemByCodeQuery, eachItem.ItemCode, eachItem.Description, eachItem.Quantity)
